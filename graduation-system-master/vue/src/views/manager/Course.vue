    ...
    <div class="card table-view" v-if="data.user.role === 'ADMIN'">
      ...
      <el-table :data="data.tableData" stripe class="scrollable-content">
        ...
        <el-table-column
          label="课程介绍"
          prop="content"
          :show-overflow-tooltip="{ popperClass: 'course-tooltip-popper' }"
        ></el-table-column>
...
const loadByTeacherName = () => {
  const teacherName = data.user.name; // 从用户信息中获取教师姓名
  if (!teacherName) {
    ElMessage.error('未获取到教师姓名');
    return;
  }

  request.get('/course/selectByTeacherName', {
    params: {
      teacherName: teacherName,
      name: data.name // 保留课程名称查询条件
    },
  }).then(res => {
    data.tableData = res.data?.map(course => ({
      ...course,
      imageUrl: '/default-course-image.jpg' // 设置默认图片
    })) || [];
    console.log('教师课程数据:', data.tableData);
  });
};
...
:global(.course-tooltip-popper .el-tooltip__content) {
  white-space: normal !important;
  max-width: 420px !important;
}
